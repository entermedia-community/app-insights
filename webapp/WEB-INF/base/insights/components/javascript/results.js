...
{ 
	...
	showInsight = function(id,pagenum)
	{
		if (id) {
			var mainmedia = $("#main-media-viewer");
			var resultsdiv = $("#resultsdiv");
		...
		if( link == null )
		{
			 link = componenthome + "/gridsample/preview/insight.html";	
		}
		var	hitssessionid = resultsdiv.data("hitssessionid");
		var params = {embed:true,id:id,hitssessionid:hitssessionid,oemaxlevel:1};
		if( pagenum != null )
		{
			params.pagenum = pagenum; //Do we use this for anything?
		}
	...
	lQuery('a.insightplayer').livequery('click',function(e)
			{
				e.preventDefault();
				var link = $(this);
				var id = link.data("id");
				var pagenum = link.data("pagenum"); 
				showInsight(id,pagenum);
				return false;
			});
